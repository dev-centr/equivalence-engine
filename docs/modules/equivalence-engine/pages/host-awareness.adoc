...
image::playtime-bootstrap.svg[alt="Deadlock cycle crossed out; snapshot then Equivalence Engine bind",role=themed-svg]
...
image::playtime-growth-ratchet.svg[alt="Rules to CI bake to shipped PlayTime to more playbooks back to new catalog rows",role=themed-svg]
...
.Columns, not a cube
image::playtime-facets-not-lattice.svg[alt="Catalog table versus a forbidden path-segment lattice",role=themed-svg]

.One suitcase, no costumes
image::playtime-argv.svg[alt="argv box for winget install; dashed shell coats are not needed",role=themed-svg]
